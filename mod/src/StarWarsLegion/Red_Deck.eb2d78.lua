function onload(save_state)
    ...
    optionButton = getObjectFromGUID("4df750")

    deckZone = getObjectFromGUID(listBuilder.redDeckZoneGUID)
    deckMount = getObjectFromGUID(listBuilder.redDeckMountGUID)
...
function checkDeck()
    ...
    if gotDeck == true then
      gameControllerObj.call("defineBattlefieldMenu", {
        deck     = battleDeckObj,
        scenario = self.getVar("selectedScenario"),
      })
    else
